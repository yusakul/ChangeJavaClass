import org.gjt.jclasslib.io.ClassFileWriter;
import org.gjt.jclasslib.structures.ClassFile;
import org.gjt.jclasslib.structures.Constant;
import org.gjt.jclasslib.structures.FieldInfo;
import org.gjt.jclasslib.structures.MethodInfo;
import org.gjt.jclasslib.structures.attributes.CodeAttribute;
import org.gjt.jclasslib.structures.constants.ConstantStringInfo;
import org.gjt.jclasslib.structures.constants.ConstantUtf8Info;
import org.gjt.jclasslib.structures.AttributeInfo;

import java.io.DataInput;
import java.io.DataInputStream;
import java.io.File;
import java.io.FileInputStream;


public class main {
    public static void main(String[] args) throws Exception {
        String filePath = "D:\\test.class";//新建的需要進行操作的資料夾
        FileInputStream fis = new FileInputStream(filePath);

        DataInput di = new DataInputStream(fis);
        ClassFile cf = new ClassFile();
        cf.read(di);
        Constant[] infos = cf.getConstantPool();//获取常量数组
        FieldInfo[] fileInfos = cf.getFields();//获取字段数组
        MethodInfo[] methodinfos = cf.getMethods();//获取方法数组

        //修改常量 ---start
        int count = infos.length;
        for(int i = 0; i < count; i++) {
            if(infos[i] != null) {
                System.out.print(i);
                System.out.print(" = ");
                System.out.print(infos[i].getVerbose());
                System.out.print(" = ");
//				System.out.println(infos[i].getVerbose() != "");
                System.out.println(infos[i].getVerbose() != "" ? infos[i].getConstantType() : "");
                if(i == 274) {
                    //解決cast錯誤
                    //ConstantStringInfo sInfo = (ConstantStringInfo) infos[i];
                    //int strIndex = sInfo.getStringIndex();
                    //ConstantUtf8Info uInfo = (ConstantUtf8Info)infos[strIndex];
                    ConstantUtf8Info uInfo = (ConstantUtf8Info)infos[i];
                    uInfo.setString("-----BEGIN CERTIFICATE-----\\n8cLi/wOPE4XKNtA+4uo2q1np\n-----END CERTIFICATE-----\n");
                    //uInfo.setBytes("aaaaa".getBytes());
                    infos[i]=uInfo;
                }
            }
        }
        cf.setConstantPool(infos);
        //修改常量 ---end

        //修改方法
        int count_method = methodinfos.length;
        for(int i = 0; i < count_method; i++) {
            if(methodinfos[i] != null) {
                if(i == 4) {
                    MethodInfo methodInfo = methodinfos[i];
                    AttributeInfo[] attributeInfo= methodInfo.getAttributes();
                    int count_Attribute = attributeInfo.length;
                    for(int j=0;j<count_Attribute;j++) {
                        CodeAttribute ca = (CodeAttribute) attributeInfo[j];
                        byte[] codes = ca.getCode();
                        byte aimcode = codes[29];
                        if(aimcode==(byte)0x9A) {
                            byte[] myByteArray= codes;
                            myByteArray[29]= (byte) 0xA8;
                            ca.setCode(myByteArray);
                            attributeInfo[j]=ca;
                            methodinfos[i]=methodInfo;
                        }
                    }

                }
            }
        }
        cf.setMethods(methodinfos);


        fis.close();
        File f = new File(filePath);
        ClassFileWriter.writeToFile(f, cf);


    }

}
